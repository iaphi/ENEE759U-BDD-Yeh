...
main (int argc, char **argv)
{
    ...
    // Function2 sifting
    char** sifting_list_temp = malloc(net->ninputs * sizeof(char*));   //store the result in last iteration (referece for perform sifting)
    char** sifting_list_best = malloc(net->ninputs * sizeof(char*));   //store the best result in current
	int best_size = 0;
	for (int i = 0; i < net->ninputs; i++) {
        sifting_list_best[i] = net->inputs[i];
    }
    for (int sifting_iter = 0; sifting_iter < net->ninputs; sifting_iter++){
		// Copy sifting_list_best to sifting_list_temp
        for (int i = 0; i < net->ninputs; i++) {
            sifting_list_temp[i] = sifting_list_best[i];
        }

		for (int sifting_idx = 0; sifting_idx < net->ninputs; sifting_idx++) {
			...
			(void) fclose (fp);
            // Swap elements for sifting
            char* temp = sifting_list_temp[sifting_iter];
        	sifting_list_temp[sifting_iter] = sifting_list_temp[sifting_idx];
        	sifting_list_temp[sifting_idx] = temp;
			new_net->inputs = sifting_list_temp;
        ...
        if (best_size == 0 || mysize < best_size) {
            ...
            for (int i = 0; i < new_net->ninputs; i++) {
                sifting_list_best[i] = new_net->inputs[i];
            }
        }

        temp = sifting_list_temp[sifting_iter];
        sifting_list_temp[sifting_iter] = sifting_list_temp[sifting_idx];
        sifting_list_temp[sifting_idx] = temp;
