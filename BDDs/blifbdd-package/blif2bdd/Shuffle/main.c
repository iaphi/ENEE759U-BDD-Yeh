...
main (int argc, char **argv)
{
    ...
    // Function Shuffling
    char** shuffle_list_temp = malloc(net->ninputs * sizeof(char*));   //store the result in last iteration (referece for perform sifting)
    char** shuffle_list_best = malloc(net->ninputs * sizeof(char*));   //store the best result in current
	int best_size = 0;
    for (int iter = 0; iter < net->ninputs*net->ninputs; iter++){
		// Copy sifting_list_best to sifting_list_temp
        for (int i = 0; i < net->ninputs; i++) {
            shuffle_list_temp[i] = net->inputs[i];
        }
            ...
            for (int shuffle_iter = 1; shuffle_iter > 0; shuffle_iter--){
                //int shuffle_iter = rand()%(net->ninputs);
                int shuffle_idx = rand()%(net->ninputs);
                char* shuffle_temp = shuffle_list_temp[shuffle_iter];
                shuffle_list_temp[shuffle_iter] = shuffle_list_temp[shuffle_idx];
                shuffle_list_temp[shuffle_idx] = shuffle_temp;
            }
			new_net->inputs = shuffle_list_temp;
        
			// PIs = new_net->inputs;
			// fprintf (stdout, "\nPI are: \n");
			// for (i = 0; i < new_net->ninputs; i++) { fprintf (stdout, "\n\t%s\n", PIs[i]);
			// }
		//printf("checkpoint1\n");
		...
		//printf("checkpoint2\n");
        result = Ntr_buildDDs (new_net, manager, option, NULL);
        if (result == 0) {
            fprintf (stderr, "Cannot build DD from the BNetwork");
            exit (-1);
        }
		...
		// If the new size is better, update the best list and size
        if (best_size == 0 || mysize < best_size) {
            best_size = mysize;
            for (int i = 0; i < new_net->ninputs; i++) {
                shuffle_list_best[i] = new_net->inputs[i];
            }
        }
		//printf("checkpoint5\n");
